...
Tracking::Tracking(ORBVocabulary* pVoc, /*FramePublisher *pFramePublisher, MapPublisher *pMapPublisher,*/
...
/*
    ...
    if(mState==WORKING || mState==LOST)
        mCurrentFrame = Frame(im,timesmp,mpORBextractor,mpORBVocabulary,mK,mDistCoef);
    else
        mCurrentFrame = Frame(im,timesmp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef);
 ...
 //   mpFramePublisher->Update(this);
  ...
  //                      Rwc.at<float>(1,0),Rwc.at<float>(1,1),Rwc.at<float>(1,2),
   ...
   //     tf::Vector3 V(twc.at<float>(0), twc.at<float>(1), twc.at<float>(2));
        ofstream fout;
        fout.open("/home/lql/output/orb_output.txt", ios::app);
        fout << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << endl;
        fout.flush();
        fout.close();
        tkCount ++;
        cout <<"tkCount = " <<  tkCount << endl;
